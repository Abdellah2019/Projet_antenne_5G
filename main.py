...
def v_nm(n, m, theta, phi, d_v, d_h, lam):
    return np.exp(-1j * 2 * np.pi * ((n-1) * d_v / lam * np.cos(theta) + (m-1) * d_h / lam * np.sin(theta) * np.sin(phi)))

# Fonction pour la pondération
def w_inm(n, m, theta_i_max, phi_i_max, d_v, d_h, lam, N_v, N_h):
    return (1 / (N_v * N_h)) * np.exp(-1j * 2 * np.pi * ((n-1) * d_v / lam * np.sin(theta_i_max) - (m-1) * d_h / lam * np.cos(theta_i_max) * np.sin(phi_i_max)))

# Fonction pour le diagramme composite (renommée en A_Beam)
def A_Beam(theta, phi, A_e, N_v, N_h, d_v, d_h, lam, theta_i_max, phi_i_max):
    v = np.array([[v_nm(n, m, theta, phi, d_v, d_h, lam) for m in range(1, N_h+1)] for n in range(1, N_v+1)])
    w = np.array([[w_inm(n, m, theta_i_max, phi_i_max, d_v, d_h, lam, N_v, N_h) for m in range(1, N_h+1)] for n in range(1, N_v+1)])
    return A_e + 10 * np.log10(np.abs(np.sum(w * v))**2)
...
with tab2:
    ...
    theta_i_max = st.slider("Angle d'inclinaison max (degrés)", min_value=0, max_value=180, value=90)
    phi_i_max = st.slider("Angle d'azimut max (degrés)", min_value=0, max_value=360, value=180)
    ...
    theta_values, phi_values = np.meshgrid(theta_values, phi_values)

    # Calcul du diagramme composite
    A_Beam_values = np.vectorize(A_Beam)(theta_values[:, np.newaxis], phi_values, A_e, N_v, N_h, d_v, d_h, lam, theta_i_max_rad, phi_i_max_rad)
    # Affichage du diagramme composite en 2D
    fig, ax = plt.subplots(figsize=(10, 6))
    c = ax.contourf(phi_values, theta_values, A_Beam_values, cmap='viridis')
    ax.set_title("Diagramme composite de l'antenne")
    ax.set_xlabel("Angle φ (degrés)")
    ax.set_ylabel("Angle θ (degrés)")
